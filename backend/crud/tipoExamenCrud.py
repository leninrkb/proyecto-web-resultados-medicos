...
from schemas import TipoExamenSchema
from models import TipoExamen

def get_tipos_examenes(db:Session, skip:int=0, limit:int=100):
    return db.query(TipoExamen).offset(skip).limit(limit).all()

def get_tipo_examen_id(db:Session, id:int):
    return db.query(TipoExamen).filter(TipoExamen.id == id).first()
